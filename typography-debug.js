...
(() => {
  ...
  const groups = [
    ...
    [
      "globalDisplay",
      "Global · display fonts",
      ":where(.wordmark svg text, .utility, .anchors a, .scene-content > h2, .press-page h1, .press-page h2)",
    ],
    [
      "globalBody",
      "Global · reading fonts",
      ":where(p, li, th, td, .text-link, .gallery-controls button)",
    ],
    ["wordmark", "Whole wordmark", ".wordmark"],
    ["funk", "FUNKADELIC", ".funk-word"],
    ["astro", "ASTRONAUT", ".astro-word"],
    ["press", "Press kit link", ".navigation > .utility:not(.booking)"],
    ["booking", "Booking link", ".navigation .booking"],
    ["nav", "Navigation links", ".anchors a"],
    ["listen", "Listen heading", "#listen-title"],
    ["learn", "About heading", "#learn-title"],
    ["live", "Tour heading", "#live-title"],
    ["session", "Performance caption", ".play-area .eyebrow"],
    ["role", "Band / member label", "#member-role"],
    ["name", "Band / member name", "#member-name"],
    ["story", "Biography paragraphs", "#member-story p"],
    ["shows", "Upcoming shows label", ".tour-label"],
    ["table", "Show table headings", ".live th"],
    ["empty", "Show announcement", ".empty-title"],
    ["dates", "Show details", ".live td p:not(.empty-title)"],
    ["follow", "Show follow link", ".live .text-link"],
    ["pressTitle", "Press page title", ".press-page h1"],
    ["pressHeadings", "Press page headings", ".press-page h2"],
  ...
  const adopted = {
    wordmark: { x: 0, y: 0, rotation: 0, scaleX: 1, scaleY: 1 },
  };
  const scopes = {
    all: "",
  ...
  const fonts = {
    modak: "Modak, Georgia, serif",
    shrikhand: "Shrikhand, Georgia, serif",
    cooper: '"Cooper Black", Georgia, serif',
  ...
  const webFonts = {
    modak: "Modak",
    shrikhand: "Shrikhand",
    fraunces: "Fraunces:wght@100;200;300;400;500;600;700;800;900",
    abril: "Abril+Fatface",
    bungee: "Bungee",
    righteous: "Righteous",
  ...
  try {
    draft = validate(JSON.parse(localStorage.getItem(key)));
  } catch {
    /* A fresh draft is safe. */
  }
  function rules() {
    const result = [];
    for (const s of Object.keys(scopes))
      for (const [id, label, selector] of groups) {
        const v = draft[s][id];
        if (!v) continue;
        const css = {};
        const inherited = {
        ...
        if (v.font)
          result.push({
            label,
            selector: `${selector}, :is(${selector}) :where(*)`,
            media: scopes[s] || null,
            css: {
              "font-family": ["modak", "chicle"].includes(v.font)
                ? fonts.modak
                : "var(--body-face)",
            },
          });
        ...
        if (v.x !== undefined || v.y !== undefined)
          css.translate = `${v.x ?? inherited.x ?? 0}px ${v.y ?? inherited.y ?? 0}px`;
        if (v.rotation !== undefined) css.rotate = `${v.rotation}deg`;
        if (v.scaleX !== undefined || v.scaleY !== undefined)
          css.scale = `${v.scaleX ?? inherited.scaleX ?? 1} ${v.scaleY ?? inherited.scaleY ?? 1}`;
        if (id === "follow" && (css.translate || css.rotate || css.scale))
          css.display = "inline-block";
        if (Object.keys(css).length)
          result.push({ label, selector, media: scopes[s] || null, css });
        const glyphCSS = {};
        if (v.glyph !== undefined) glyphCSS.transform = `scaleX(${v.glyph})`;
        if (v.gap !== undefined) glyphCSS["margin-inline"] = `${v.gap}em`;
        if (Object.keys(glyphCSS).length)
          result.push({
            label,
            selector: `${selector} .display-letter`,
            media: scopes[s] || null,
            css: glyphCSS,
          });
      }
  ...
  window.legacyTypography = {
    get: () => structuredClone(draft),
    rules(data) {
      draft = validate(data);
      return rules();
    },
