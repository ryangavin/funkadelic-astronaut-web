...
export function cylinderSide(a:CylinderLayer, ar:number, b:CylinderLayer, br:number, center={x:180,y:180}) {
  ...
  const point=(p:CylinderLayer,radius:number,n:{x:number;y:number})=>`${center.x+p.x+radius*n.x} ${center.y+p.y+radius*n.y}`;
