...
const metricTicks = Array.from({ length: 1001 }, (_, mm) =>
  `M${mm} 0v${mm % 10 === 0 ? 12 : mm % 5 === 0 ? 8 : 4}`).join('');
const inchTicks = Array.from({ length: Math.floor(1000 / 25.4 * 8) + 1 }, (_, eighth) =>
  `M${eighth * 25.4 / 8} 40v-${eighth % 8 === 0 ? 10 : eighth % 4 === 0 ? 7 : 4}`).join('');
...
export function MeterStick({ inches = true }: MeterStickProps) {
  return <svg className="meter-stick" viewBox="0 0 1000 40" role="img" aria-label={`One meter stick, 100 centimeters${inches ? ', 39.37 inches' : ''}`}>
    ...
    <g className="meter-stick__numbers">
      {Array.from({ length: 101 }, (_, cm) => <text key={cm} x={cm * 10} y="19" textAnchor={cm === 0 ? 'start' : cm === 100 ? 'end' : 'middle'}>{cm}</text>)}
    </g>
    {inches && <>
      <path d={inchTicks} className="meter-stick__ticks meter-stick__inches" />
      <g className="meter-stick__numbers meter-stick__inches">
        {Array.from({ length: 40 }, (_, inch) => <text key={inch} x={inch * 25.4} y="28" textAnchor={inch === 0 ? 'start' : 'middle'}>{inch}</text>)}
      </g>
    </>}
    <text className="meter-stick__brand" x="500" y={inches ? 26 : 30} textAnchor="middle">FUNK ASTRONAUT · FIELD STANDARD · 1 m / 100 cm</text>
    <text className="meter-stick__unit" x="990" y="28" textAnchor="end">{inches ? 'in' : 'cm'}</text>
  </svg>;
}
