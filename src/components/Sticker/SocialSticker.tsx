...
/** A platform's mark in its own colours, printed clean on vinyl and die-cut around its own outline. `print="cutout"` gives it the poster's worn finish instead. */
export function SocialSticker({ platform, ink = 'brand', worn = false, print = 'flat', label, size = 72, href, ...props }: SocialStickerProps) {
  ...
  return <Sticker {...props} href={href} size={size}
    ...
    <SocialIcon platform={platform} ink={ink} size={size} worn={worn} print={print}
