...
export type SocialStickerProps = Omit<StickerProps, 'children' | 'shape' | 'overhang' | 'peelTip'> &
  Pick<SocialIconProps, 'platform' | 'ink' | 'worn' | 'label'>;

/** A platform's mark, printed on vinyl and die-cut around its own outline. */
export function SocialSticker({ platform, ink, worn = true, label, size = 72, href, ...props }: SocialStickerProps) {
  ...
  return <Sticker {...props} href={href} size={size}
    ...
    <SocialIcon platform={platform} ink={ink ?? defaultInk} size={size} worn={worn}
