...
export type DeskLight = { x: number; y: number; height: number; on: boolean; shadowStrength?: number };
...
export function useRegisterDeskLight(light: DeskLight | null) {
  ...
  const x = light?.x, y = light?.y, height = light?.height, on = light?.on, shadowStrength = light?.shadowStrength;
  useEffect(() => {
    if (!register || x === undefined || y === undefined || height === undefined || on === undefined) return;
    register({ x, y, height, on, shadowStrength });
