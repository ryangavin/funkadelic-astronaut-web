...
test('diagram derives exact eye, center, gaze and lamp coordinates in floor-relative millimetres',()=>{
  for(const tilt of [-10,0,15]) {
    ...
    near(scene.eye.x,0);near(scene.eye.y,650);near(scene.eye.z,1650);near(scene.center.y,400);near(scene.center.z,750);near(scene.tilt,tilt);
